...
class EffectParameterBox(BoxLayout):
	def __init__(self,paramName):
		super(EffectParameterBox, self).__init__()
		name_label = Label( text=(paramName) )
...
class EffectsParameterColumn(GridLayout):
	def __init__(self):
		super(EffectsParameterColumn, self).__init__()
		labels = [ "Effect Level", "Distortion", "Feedback" ]
		for i in labels:
			c = EffectParameterBox(i)
			self.add_widget( c )
...
class EffectsColumn(BoxLayout):
	def __init__(self):
		super(EffectsColumn, self).__init__()
...
class SettingsPopup(FloatLayout):
	def __init__(self):
		...
		self.settings_button = Button(text="Settings", size_hint=(.25,.05), pos_hint={'x': .75, 'y': .95})
		self.settings_button.bind(on_press=self.settings_clicked)
		self.add_widget(self.settings_button)

	def settings_clicked(self, obj):
		popup = Popup(title='PMEAS - Settings', content=Label(text='Here is where the settings will go'),auto_dismiss=True)
		popup.open()
